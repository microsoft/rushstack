...
export async function runAsync(runScriptOptions: IRunScriptOptions): Promise<void> {
  ...
  const apiDocumenterJsonPath: string = `${buildFolderPath}/temp/configs/api-documenter.json`;
    ...
    afterApiExtractorAsync: async (scenarioFolderName: string) => {
      ...
      const childProcess: ChildProcess = Executable.spawn(
        ...
        [
          ...
          '--output-folder',
          `${buildFolderPath}/temp/etc/${scenarioFolderName}/markdown`
        ],
        {
          stdio: 'inherit',
          // api-documenter will find api-documenter.json in this folder:
          currentWorkingDirectory: `${buildFolderPath}/temp/configs`
        }
      );
