...
# This script verifies that temp project tarballs work correctly with yarn package manager

set -e

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
TEST_REPO_DIR="$SCRIPT_DIR/temp/yarn-test-repo"
RUSHSTACK_ROOT="$( cd "$SCRIPT_DIR/../.." && pwd )"

echo "=========================================="
echo "Rush Yarn Mode Integration Test"
echo "=========================================="
echo ""
echo "This test verifies that tar 7.x changes work correctly with yarn package manager"
...
echo "Initializing Rush repo with yarn mode..."
node "$RUSHSTACK_ROOT/apps/rush/lib/start.js" init --overwrite-existing

# Configure rush.json to use yarn
echo "Configuring rush.json for yarn mode..."
cat > rush.json << 'EOF'
{
...
echo ""
echo "=========================================="
echo "✓ Yarn Mode Integration Test PASSED"
echo "=========================================="
echo ""
echo "The tar 7.x changes work correctly with yarn mode:"
echo "  - Temp project tarballs created successfully"
