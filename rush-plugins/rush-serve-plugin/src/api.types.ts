...
export interface IWebSocketSyncEventMessage {
  event: 'sync';
  /**
   * Static graph definition (one entry per operation in the graph).
   */
 ...
 */
export interface IWebSocketSyncGraphStateEventMessage {
  event: 'sync-graph-state';
  graphState: IWebSocketSyncEventMessage['graphState'];
}
