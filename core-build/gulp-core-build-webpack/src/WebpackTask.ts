...
export class WebpackTask<TExtendedConfig = {}> extends GulpTask<IWebpackTaskConfig & TExtendedConfig> {
  ...
  public isEnabled(buildConfig: IBuildConfig): boolean {
    return (
      super.isEnabled(buildConfig) && this.taskConfig.configPath !== null // tslint:disable-line:no-null-keyword
    );
  }
      ...
      if (webpackConfig) {
        ...
        webpack(webpackConfig, (error, stats) => {
          ...
          if (statsResult.warnings && statsResult.warnings.length) {
            ...
            const warningSuppressionRegexes = (this.taskConfig.suppressWarnings || []).map(
              (regex: string) => {
