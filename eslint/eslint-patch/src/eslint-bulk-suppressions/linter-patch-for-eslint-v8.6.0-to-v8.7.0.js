 ...
 * methods of the original Linter class with the patched Linter class.
 *
...
const path = require('path'),
  ...
  espree = requireFromPathToLinterJS('espree'),
  merge = requireFromPathToLinterJS('lodash.merge'),
  pkg = requireFromPathToLinterJS('../../package.json'),
  astUtils = requireFromPathToLinterJS('../shared/ast-utils'),
  {
    Legacy: { ConfigOps, ConfigValidator, environments: BuiltInEnvironments }
  } = requireFromPathToLinterJS('@eslint/eslintrc/universal'),
  Traverser = requireFromPathToLinterJS('../shared/traverser'),
  { SourceCode } = requireFromPathToLinterJS('../source-code'),
  CodePathAnalyzer = requireFromPathToLinterJS('./code-path-analysis/code-path-analyzer'),
  applyDisableDirectives = requireFromPathToLinterJS('./apply-disable-directives'),
  ConfigCommentParser = requireFromPathToLinterJS('./config-comment-parser'),
  NodeEventGenerator = requireFromPathToLinterJS('./node-event-generator'),
  createReportTranslator = requireFromPathToLinterJS('./report-translator'),
  Rules = requireFromPathToLinterJS('./rules'),
  createEmitter = requireFromPathToLinterJS('./safe-emitter'),
  SourceCodeFixer = requireFromPathToLinterJS('./source-code-fixer'),
  timing = requireFromPathToLinterJS('./timing'),
  ruleReplacements = requireFromPathToLinterJS('../../conf/replacements.json');
const { getRuleFromConfig } = requireFromPathToLinterJS('../config/flat-config-helpers');
const { FlatConfigArray } = requireFromPathToLinterJS('../config/flat-config-array');

const debug = requireFromPathToLinterJS('debug')('eslint:linter');
...
const globals = requireFromPathToLinterJS('../../conf/globals');
// --- END MONKEY PATCH ---
...
function normalizeVerifyOptions(providedOptions, config) {
  ...
  return {
    ...
    warnInlineConfig:
      disableInlineConfig && !ignoreInlineConfig ? `your config${configNameOfNoInlineConfig}` : null,
...
function markVariableAsUsed(scopeManager, currentNode, languageOptions, name) {
  ...
  const hasGlobalReturn =
    (parserOptions.ecmaFeatures && parserOptions.ecmaFeatures.globalReturn) || sourceType === 'commonjs';
  const specialScope = hasGlobalReturn || sourceType === 'module';
  const currentScope = getScope(scopeManager, currentNode);

  // Special Node.js scope means we need to start one level deeper
  const initialScope =
    currentScope.type === 'global' && specialScope ? currentScope.childScopes[0] : currentScope;
...
function getRule(slots, ruleId) {
  return (
    (slots.lastConfigArray && slots.lastConfigArray.pluginRules.get(ruleId)) || slots.ruleMap.get(ruleId)
  );
}
