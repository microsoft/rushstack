# Agent coding contract — @rushstack/rush-daemon-transport
...
- This package owns transport mechanics only: path derivation, sockets/pipes, and lockfiles. Protocol schemas live in `@rushstack/rush-daemon-protocol`; presentation lives in `@rushstack/rush-terminal-renderer`. No `rush-lib` dependency.