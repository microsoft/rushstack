...
function _ensureCertUtilExePath(terminal: Terminal): string | undefined {
  if (!_certutilExePath) {
...
function _tryTrustCertificate(certificatePath: string, terminal: Terminal): boolean {
  switch (process.platform) {
    case 'win32':
      const certutilExePath: string | undefined = _ensureCertUtilExePath(terminal);
      if (!certutilExePath) {
...
function _trySetFriendlyName(certificatePath: string, terminal: Terminal): boolean {
...
function _ensureCertificateInternal(terminal: Terminal, certificateStore: CertificateStore): void {
  ...
  if (_tryTrustCertificate(tempCertificatePath, terminal)) {
    ...
    if (!_trySetFriendlyName(tempCertificatePath, terminal)) {
  ...
  switch (process.platform) {
    ...
    default:
      ...
      terminal.writeLine(
        ...
        `root certification authorities: "${certificateStore.certificatePath}". The ` +
  ...
  if (certificateStore.certificateData && certificateStore.keyData) {
    if (!_certificateHasSubjectAltName(certificateStore.certificateData)) {
      ...
      if (canGenerateNewCertificate) {
        ...
        _ensureCertificateInternal(terminal, certificateStore);
      }
    }
