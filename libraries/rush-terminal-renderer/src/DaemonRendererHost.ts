...
export class DaemonRendererHost {
  ...
  public handleLogChunk(operationId: string, stream: 'stdout' | 'stderr', chunk: Buffer): void {
    ...
    this._streams.writeChunk(operationId, { kind: toChunkKind(stream), text: chunk.toString('utf8') });
  }
