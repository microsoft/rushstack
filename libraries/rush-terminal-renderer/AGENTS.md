# Agent coding contract — @rushstack/rush-terminal-renderer
...
- The renderer interface mirrors `@rushstack/reporter`'s `IReporter` so the real `default`/`ai`/`plaintext` reporters drop in unchanged at the reporter reconciliation. No `rush-lib` dependency.