...
describe('experimental native graph over daemon transport', () => {
  ...
  it('reports cold state without creating a graph or running scripts', async () => {
    ...
    for (const argv of [['watch'], ['pause'], ['resume'], ['scope-in', '--project', 'a'], ['invalidate', '--project', 'a']]) {
      expect((await fixture.graphAsync(...argv)).terminal).toMatchObject({
        kind: 'requestRejected', payload: { code: 'routingFailed', message: expect.stringContaining('uninitialized') }
      });
    }
  ...
  it('maps exact selectors to native dependency-safe enablement and validates all selectors first', async () => {
    ...
    expect(initial.operations).toEqual([
      { operationId: 'a (compile)', projectName: 'a', phaseName: '_phase:compile', enabled: true, status: 'SUCCESS', dependencyIds: [] },
      { operationId: 'b (compile)', projectName: 'b', phaseName: '_phase:compile', enabled: true, status: 'SUCCESS', dependencyIds: ['a (compile)'] },
      { operationId: 'c (compile)', projectName: 'c', phaseName: '_phase:compile', enabled: false, status: 'SKIPPED', dependencyIds: [] }
    ]);
    ...
    for (const argv of [
      ...
      ['scope-in'], ['show', '--project', 'a'], ['unknown'], ['scope-out', '--operation']
    ]) {
      expect((await fixture.graphAsync(...argv)).terminal).toMatchObject({
        kind: 'requestRejected', payload: { code: 'invalidRequest' }
      });
  ...
  it('rejects a disabled gate and malformed reserved requests before the production resolver', async () => {
    ...
    for (const overrides of invalidRequests) {
      expect((await fixture.runAsync(['daemon', 'graph', 'show'], overrides)).terminal.kind).toBe('requestRejected');
    }
    expect(fixture.session.operationGraph).toBeUndefined();
    expect(fixture.runs()).toEqual([]);
  });
  ...
  it('observes real builds, invalidations and idle without a lease, and cleans cancel/disconnect subscribers', async () => {
    ...
    try {
      ...
      expect((await client.readTerminalAsync(request.requestId)).terminal).toMatchObject({
        kind: 'requestResult', payload: { outcome: 'aborted', aborted: true, exitCode: 130 }
      });
  ...
  it('coalesces updates to one pending notification and removes resources immediately on cancellation', async () => {
    ...
    const waiting = changes.nextAsync().then((value) => { delivered = true; return value; });
  ...
  it('bounds a backpressured wire watch and cancels it without blocking another client', async () => {
    ...
    const writer = jest.spyOn(DaemonWireRequestClient.prototype, 'writeEventAsync').mockImplementation(
      async function (this: DaemonWireRequestClient, event: IDaemonEventEnvelope): Promise<void> {
    ...
    try {
      ...
      expect(observer.subscriberCount).toBe(1);
      expect(responseSnapshot(await fixture.graphAsync('pause'))).toMatchObject({ pauseNextIteration: true });
      await client.sendControlAsync({ kind: 'requestCancel', payload: { requestId: request.requestId } });
      ...
      release.resolve();
      expect((await client.readTerminalAsync(request.requestId)).terminal).toMatchObject({
        kind: 'requestResult', payload: { outcome: 'aborted', exitCode: 130 }
      });
    } finally {
      release.resolve();
      writer.mockRestore();
  ...
  it('queues mutations behind an active build and reports no-wait admission failure', async () => {
    ...
    try {
      ...
      expect(rejected.terminal).toMatchObject({
        kind: 'requestResult', payload: { outcome: 'failure', admissionErrorCode: 'no-wait' }
      });
