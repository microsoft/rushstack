...
(process.platform === 'linux' ? describe : describe.skip)('native mutation cleanup ownership', () => {
  ...
  it.each([
    ...
    'retains ownership after %s worker join %s failure, even after the result drains',
    async (command, kind) => {
      ...
      jest
        ...
        .mockImplementation((pid, signal) =>
          workerPid !== undefined && pid === -workerPid && signal === 0 ? true : originalKill(pid, signal)
        );
      ...
      jest.spyOn(nativeChildProcess, 'execFile').mockImplementation((...args) => {
        ...
        callback(
          kind === 'inspection'
            ? Object.assign(new Error('Cannot inspect owned worker'), { code: 'ENOENT' })
            : null,
          kind === 'inspection' ? '' : 'S\n',
          ''
        );
      ...
      const detail: string =
        kind === 'inspection' ? 'Cannot inspect owned worker' : 'did not exit within 25ms';
      expect(result.terminal).toMatchObject({
        ...
        payload: {
          ...
          outcome: 'failure',
          aborted: false,
          errorMessage: expect.stringContaining(detail)
        }
      });
      ...
      expect(
        spawn.mock.calls.filter(
          ([, args]) =>
            Array.isArray(args) &&
            args.some((arg) => typeof arg === 'string' && arg.endsWith('/NativeMutationWorker.js'))
        )
      ).toHaveLength(1);
      await expect(fixture.host.restartCompleted).rejects.toBeInstanceOf(
      ...
      expect(
        spawn.mock.calls.some(
          ([, args]) =>
            Array.isArray(args) &&
            args.some((arg) => typeof arg === 'string' && arg.endsWith('/runDaemonStartup.js'))
        )
      ).toBe(false);
    }
  );
