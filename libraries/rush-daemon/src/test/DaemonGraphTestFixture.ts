...
import {
  DaemonFrameType, decodeDaemonEventFrame, RUSHD_GRAPH_SNAPSHOT,
  type IDaemonFrame, type IDaemonGraphSnapshotPayload, type IDaemonRequestEnvelope
...
import { createWireEnvelope, DaemonRequestWireClient, type ITerminalExchange } from './DaemonRequestWireTestUtilities';
...
export class DaemonGraphTestFixture implements AsyncDisposable {
  ...
  public readonly environment: Record<string, string> = {
    ...Object.fromEntries(Object.entries(process.env).filter((pair): pair is [string, string] => pair[1] !== undefined))
  };
  private _nextId: number = 0;

  public static async createAsync(): Promise<DaemonGraphTestFixture> {
    const fixture: DaemonGraphTestFixture = new DaemonGraphTestFixture();
    try {
      fixture.write('rush.json', JSON.stringify({
        rushVersion: Rush.version, npmVersion: '10.0.0', projectFolderMinDepth: 1,
      ...
      fixture.write('common/config/rush/command-line.json', JSON.stringify({
      ...
      for (const name of ['a', 'b', 'c']) {
        fixture.write(`${name}/package.json`, JSON.stringify({
          name, version: '1.0.0', dependencies: name === 'b' ? { a: '1.0.0' } : {},
          scripts: { '_phase:compile': 'node build.cjs' }
        }));
        fixture.write(`${name}/input.txt`, 'one');
        fixture.write(`${name}/build.cjs`,
  ...
  private async _startAsync(): Promise<void> {
    this.host = await RushDaemonHost.startAsync({
      repoRoot: this.folder, rushVersion: Rush.version, daemonVersion: 'graph-test',
  ...
  public async connectAsync(): Promise<DaemonRequestWireClient> {
    const client: DaemonRequestWireClient = await DaemonRequestWireClient.connectAsync(this.host.paths.socketPath);
  ...
  public envelope(argv: string[], overrides: Partial<IDaemonRequestEnvelope> = {}): IDaemonRequestEnvelope {
    return createWireEnvelope(`graph-${++this._nextId}`, argv[0], this.folder, {
      argv, commandOrigin: 'built-in',
      environment: argv[0] === 'daemon' ? { ...this.environment, RUSH_DAEMON_EXPERIMENTAL: '1' } : this.environment,
      terminal: { isTTY: false, supportsColor: false }, ...overrides
    });
  }

  public async runAsync(argv: string[], overrides: Partial<IDaemonRequestEnvelope> = {}): Promise<ITerminalExchange> {
...
export function responseSnapshot(exchange: ITerminalExchange): IDaemonGraphSnapshotPayload['snapshot'] {
  ...
  expect(exchange.frames.some((frame) =>
