...
const _searchListPromptImpl: (config: ISearchListConfig<unknown>) => Promise<unknown> =
  createPrompt<unknown, ISearchListConfig<unknown>>((config, done) => {
    ...
    if (status === 'done') {
      return `${prefix} ${config.message} ${Colorize.cyan(selectedChoice?.filterText ?? '')}`;
    }
 ...
 */
export function searchListPrompt<Value>(config: ISearchListConfig<Value>): Promise<Value> {
  return _searchListPromptImpl(config as ISearchListConfig<unknown>) as Promise<Value>;
}
