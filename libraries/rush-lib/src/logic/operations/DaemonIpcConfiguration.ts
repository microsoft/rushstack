...
export async function resolveDaemonIpcConfigurationAsync(
  ...
  if (
    ...
    throw new Error(
      'daemonIpc requires a project-relative Node entryPoint and literal string args without NUL.'
    );
  }
  ...
  if (!Path.isUnder(lexicalRoot, project)) {
    throw new Error(
      'daemonIpc.entryPoint must be inside a dedicated implementation subdirectory of the project.'
    );
  }
  ...
  if (!Path.isUnder(root, project) || !Path.isUnder(entryPoint, root)) {
    throw new Error(
      'daemonIpc implementation must remain physically inside its project and dedicated directory.'
    );
  }
  ...
  const visitAsync = async (folder: string, depth: number): Promise<void> => {
    if (depth > MAX_IMPLEMENTATION_DEPTH)
      throw new Error('daemonIpc implementation exceeds 16 directory levels.');
    const entries: Dirent[] = await fs.readdir(folder, { withFileTypes: true });
    entries.sort((left, right) => (left.name < right.name ? -1 : left.name > right.name ? 1 : 0));
    for (const item of entries) {
      if (++entryCount > MAX_IMPLEMENTATION_ENTRIES)
        throw new Error('daemonIpc implementation exceeds 256 entries.');
      ...
      if (item.isDirectory()) {
        ...
        if (byteCount + stat.size > MAX_IMPLEMENTATION_BYTES)
          throw new Error('daemonIpc implementation exceeds 8 MiB.');
