...
function createGraph(
  graphOptions: IOperationGraphOptions,
...
describe('OperationGraph', () => {
  ...
  describe('Error logging', () => {
    beforeEach(() => {
      ...
      graphIterationOptions = {};
    });

    it('printedStderrAfterError', async () => {
      const graph: OperationGraph = createGraph(
        graphOptions,
      ...
      expect(graph.status).toEqual(OperationStatus.Failure);
  ...
  describe('Aborting', () => {
    it('Aborted operations abort', async () => {
      ...
      graph.hooks.beforeExecuteIterationAsync.tapPromise(
        'test',
        (): Promise<void> => graph.abortCurrentIterationAsync()
      );
      const result: IExecutionResult = await graph.executeAsync(graphIterationOptions);
      expect(result.status).toEqual(OperationStatus.Aborted);
      expect(graph.status).toEqual(OperationStatus.Aborted);
  ...
  describe('Blocking', () => {
    it('Failed operations block', async () => {
      ...
      const graph: OperationGraph = new OperationGraph(new Set([failingOperation, blockedOperation]), {
        quietMode: false,
        debugMode: false,
        parallelism: 1,
        allowOversubscription: true,
  ...
  describe('onExecutionStatesUpdated hook', () => {
    ...
    it('fires state updates for status transitions (captures snapshot statuses)', async () => {
      ...
      graph.hooks.onExecutionStatesUpdated.tap('test', (records) => {
  ...
  describe('Warning logging', () => {
    describe('Fail on warning', () => {
      ...
      it('Logs warnings correctly', async () => {
        ...
        expect(graph.status).toEqual(OperationStatus.SuccessWithWarning);
        expect(result.operationResults.size).toEqual(1);
        const firstResult: IOperationExecutionResult | undefined = result.operationResults
          .values()
          .next().value;
  ...
  describe('Cobuild logging', () => {
    ...
    function createCobuildGraph(
      cobuildExecutionManagerOptions: IOperationGraphOptions,
      ...
      graph.hooks.afterExecuteOperationAsync.tapPromise('TestPlugin', async (record) => {
    ...
    it('logs cobuilt operations correctly with --timeline option', async () => {
      const graph: OperationGraph = createCobuildGraph(
        graphOptions,
        (name) =>
          new MockOperationRunner(
            `${name} (success)`,
  ...
  describe('Manual iteration mode', () => {
    it('queues an iteration in manual mode and does not auto-execute until executeScheduledIterationAsync is called', async () => {
      ...
      try {
        const options: IOperationGraphOptions = {
          quietMode: false,
          debugMode: false,
          parallelism: 1,
          allowOversubscription: true,
        ...
        const graph: OperationGraph = new OperationGraph(new Set([op]), options);
        ...
        const executed: boolean = await graph.executeScheduledIterationAsync();
        expect(executed).toBe(true);
        expect(runFn).toHaveBeenCalledTimes(1);
        expect(graph.hasScheduledIteration).toBe(false);
        // After execution status should be Success
        expect(graph.status).toBe(OperationStatus.Success);
      } finally {
        jest.useRealTimers();
      }
    });
    ...
    it('does not queue an iteration if all operations are disabled (no enabled operations)', async () => {
      ...
      const graph: OperationGraph = new OperationGraph(new Set([disabledOp]), options);
  ...
  describe('Terminal destination APIs', () => {
    ...
    it('addTerminalDestination causes new destination to receive output', async () => {
      ...
      graph.addTerminalDestination(extraDest);
    ...
    it('removeTerminalDestination closes destination by default and stops further output', async () => {
      ...
      const first = await graph.executeAsync(graphIterationOptions);
      ...
      const removed = graph.removeTerminalDestination(extraDest);
      ...
      const second = await graph.executeAsync(graphIterationOptions);
    ...
    it('removeTerminalDestination with close=false does not close destination but still stops further output', async () => {
      ...
      const removed = graph.removeTerminalDestination(extraDest, false);
      expect(removed).toBe(true);
    ...
    it('removeTerminalDestination returns false when destination not found', () => {
      ...
      const graph = createGraph(graphOptions, new MockOperationRunner('noop'));
      const removed = graph.removeTerminalDestination(unknown);
...
describe('invalidateOperations', () => {
  it('invalidates a specific operation and updates graph status', async () => {
    ...
    graph.hooks.onInvalidateOperations.tap('test', (ops: Iterable<Operation>, reason: string | undefined) => {
...
describe('closeRunnersAsync', () => {
  ...
  it('invokes closeAsync on runners and triggers onExecutionStatesUpdated hook', async () => {
    ...
    const graph: OperationGraph = createGraph(localOptions, runner);
  ...
  it('only closes specified runners when operations iterable provided', async () => {
    ...
    await graph.closeRunnersAsync([op1]);
    expect(runner1.closeAsync).toHaveBeenCalledTimes(1);
    expect(runner2.closeAsync).not.toHaveBeenCalled();
  });
});

describe('Graph state change notifications', () => {
  function createGraphForStateTests(overrides: Partial<IOperationGraphOptions> = {}): OperationGraph {
    const baseOptions: IOperationGraphOptions = {
      quietMode: false,
      debugMode: false,
      parallelism: 2,
      maxParallelism: 4,
    ...
    return new OperationGraph(new Set(), { ...baseOptions, ...overrides });
  }
  ...
  it('invokes callback when a single property changes', async () => {
    const graph: OperationGraph = createGraphForStateTests();
    const calls: IOperationGraph[] = [];
    graph.hooks.onGraphStateChanged.tap('test', (m) => calls.push(m));
    graph.debugMode = true;
    expect(calls.length).toBe(0);
    await flushNextTick();
    expect(calls.length).toBe(1);
    expect(graph.debugMode).toBe(true);
  });
  ...
  it('debounces multiple property changes in the same tick', async () => {
    ...
    expect(graph.pauseNextIteration).toBe(true);
    expect(graph.parallelism).toBe(3);
  });
  ...
  it('clamps parallelism to configured bounds and invokes callback only when value changes', async () => {
    ...
    graph.parallelism = 10;
    await flushNextTick();
    expect(graph.parallelism).toBe(4);
    expect(calls.length).toBe(1);

    // Set to same clamped value -> no new callback
    graph.parallelism = 10; // still clamps to 4, unchanged
    await flushNextTick();
    expect(calls.length).toBe(1);
  ...
  it('pauseNextIteration change triggers callback only when value changes', async () => {
    ...
    await flushNextTick();
    expect(calls.length).toBe(2);
    expect(graph.pauseNextIteration).toBe(false);
  });
});
...
describe('setEnabledStates', () => {
  ...
  function createGraphWithOperations(ops: Operation[]): OperationGraph {
    return new OperationGraph(new Set(ops), {
      quietMode: false,
      debugMode: false,
      parallelism: 1,
      allowOversubscription: true,
  ...
  it('safe enable expands dependencies', () => {
    ...
    b.enabled = false;
    c.enabled = false;
    const graph = createGraphWithOperations([a, b, c]);
    const calls: ReadonlySet<Operation>[] = [];
    graph.hooks.onEnableStatesChanged.tap('test', (ops) => calls.push(new Set(ops)));
    const changed = graph.setEnabledStates([a], true, 'safe');
  ...
  it('safe disable disables entire dependency subtree when not required elsewhere', () => {
    ...
    const changedA = graph.setEnabledStates([a], false, 'safe');
  ...
  it('safe ignore-dependency-changes sets requested and dependencies to ignore state, respects per-op flag', () => {
    ...
    const calls: ReadonlySet<Operation>[] = [];
    graph.hooks.onEnableStatesChanged.tap('test', (ops) => calls.push(new Set(ops)));
    const changed = graph.setEnabledStates([a], 'ignore-dependency-changes', 'safe');
    expect(changed).toBe(true);
  ...
  it('unsafe mode only mutates provided operations', () => {
    ...
    const calls: ReadonlySet<Operation>[] = [];
    graph.hooks.onEnableStatesChanged.tap('test', (ops) => calls.push(new Set(ops)));
    const changed = graph.setEnabledStates([b], false, 'unsafe');
    expect(changed).toBe(true);
