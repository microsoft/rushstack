...
describe(BuildPlanPlugin.name, () => {
  ...
  ): Promise<OperationExecutionManager> {
  ...
  describe('build plan debugging', () => {
    it('should generate a build plan', async () => {
      ...
      const executionManager = await testCreateOperationsAsync(
        hooks,
        buildCommand.phases,
      ...
      await executionManager.executeAsync({ inputsSnapshot: inputsSnapshot as IInputsSnapshot });
