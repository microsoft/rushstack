...
export class LegacySkipPlugin implements IPhasedCommandPlugin {
  ...
  public apply(hooks: PhasedCommandHooks): void {
    ...
    hooks.beforeExecuteOperations.tapPromise(
      PLUGIN_NAME,
      async (context: ReadonlyMap<Operation, IOperationExecutionResult>): Promise<void> => {
        let logGitWarning: boolean = false;

        await Async.forEachAsync(context.values(), async (record: IOperationExecutionResult) => {
    ...
    hooks.afterExecuteOperation.tapPromise(
      ...
      async (
        ...
        if (!record.operation.runner!.supportsIncremental) {
          // This operation doesn't support skip detection.
          return;
        }
