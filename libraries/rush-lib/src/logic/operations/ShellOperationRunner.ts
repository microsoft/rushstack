  ...
  private async _executeAsync(context: IOperationRunnerContext): Promise<OperationStatus> {
    ...
    try {
      ...
      const splitterTransform: SplitterTransform = new SplitterTransform({
        destinations: [context.terminalWritable, removeColorsTransform]
      });

      const collatedTerminal: CollatedTerminal = new CollatedTerminal(splitterTransform);
