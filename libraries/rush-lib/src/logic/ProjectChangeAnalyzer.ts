...
export interface IGetChangedProjectsOptions {
   ...
   * - Changes to `package.json` produced by a version bump, including dependency range updates for
   *   other locally bumped projects
...
export class ProjectChangeAnalyzer {
  ...
  public async getChangedProjectsAsync(
    ...
    const packageJsonChanges: Map<RushConfigurationProject, IPackageJsonChange> = excludeVersionOnlyChanges
      ? await getPackageJsonChangesAsync(changesByProject, repoRoot, this._git)
      : new Map();
    const bumpedProjectVersions: Map<string, string> = new Map();
    for (const [project, packageJsonChange] of packageJsonChanges) {
      if (packageJsonChange.oldPackageJson.version !== packageJsonChange.newPackageJson.version) {
        bumpedProjectVersions.set(project.packageName, packageJsonChange.newPackageJson.version);
      }
    }
    ...
    await Async.forEachAsync(
      ...
      async ([project, projectChanges]) => {
        ...
        // Filter out generated version bump changes in package.json and changelog files.
        for (const filePath of filteredChanges.keys()) {
          ...
          // Check if this is package.json at project root with changes generated by the version bump
          if (projectRelativePath === '/package.json') {
            const packageJsonChange: IPackageJsonChange | undefined = packageJsonChanges.get(project);
            const isVersionBumpChange: boolean =
              !!packageJsonChange &&
              isPackageJsonVersionBumpChange(
                packageJsonChange.oldPackageJson,
                packageJsonChange.newPackageJson,
                bumpedProjectVersions
              );
...
const dependencyFieldNames: ReadonlyArray<keyof Pick<
  IPackageJson,
  'dependencies' | 'devDependencies' | 'peerDependencies'
>> = ['dependencies', 'devDependencies', 'peerDependencies'];

async function getPackageJsonChangesAsync(
  changesByProject: ReadonlyMap<RushConfigurationProject, ReadonlyMap<string, IFileDiffStatus>>,
  repoRoot: string,
  git: Git
): Promise<Map<RushConfigurationProject, IPackageJsonChange>> {
  const packageJsonChanges: Map<RushConfigurationProject, IPackageJsonChange> = new Map();
  await Async.forEachAsync(
    changesByProject,
    async ([project, projectChanges]) => {
      ...
      try {
        ...
        const oldPackageJson: IPackageJson = JSON.parse(oldPackageJsonContent);
        const newPackageJson: IPackageJson = JSON.parse(newPackageJsonContent);
        if (
          ...
          packageJsonChanges.set(project, { oldPackageJson, newPackageJson });
        }
      } catch (error) {
        // A package.json that cannot be read or parsed must remain a substantive change.
      }
    },
    { concurrency: 10 }
  );
...
/**
 * Determines whether a package.json differs only by its version and dependency range rewrites that
 * Rush would generate for other locally bumped projects.
 */
export function isPackageJsonVersionBumpChange(
  oldPackageJson: IPackageJson,
  newPackageJson: IPackageJson,
  bumpedProjectVersions: ReadonlyMap<string, string>
): boolean {
  if (
    typeof oldPackageJson.version !== 'string' ||
    typeof newPackageJson.version !== 'string' ||
  ...
  for (const dependencyFieldName of dependencyFieldNames) {
    ...
    if (
      !oldDependencies ||
      !newDependencies ||
      Object.keys(oldDependencies).length !== Object.keys(newDependencies).length
    ) {
      return false;
    }

    for (const [dependencyName, oldDependencyVersion] of Object.entries(oldDependencies)) {
      const newDependencyVersion: string | undefined = newDependencies[dependencyName];
      ...
      const bumpedProjectVersion: string | undefined = bumpedProjectVersions.get(dependencyName);
      if (!bumpedProjectVersion) {
        return false;
      }

      try {
        const expectedDependencyVersion: string = PublishUtilities.getNewDependencyVersion(
          oldDependencies,
          dependencyName,
          bumpedProjectVersion
        );
        if (newDependencyVersion !== expectedDependencyVersion) {
          return false;
        }
      } catch (error) {
        return false;
