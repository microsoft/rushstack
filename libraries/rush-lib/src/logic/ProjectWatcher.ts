...
export class ProjectWatcher {
  ...
  private readonly _executionManager: IOperationExecutionManager;
  ...
  public constructor(options: IProjectWatcherOptions) {
    const { executionManager, debounceMs, rushConfiguration, terminal, initialSnapshot } = options;
    ...
    executionManager.hooks.onWaitingForChanges.tap('ProjectWatcher', () => {
      this._startWatching();
    });

    // Dispose stdin listener when session aborts
    executionManager.abortController.signal.addEventListener(
      'abort',
      () => {
        this._disposeStdin();
      },
  ...
  private _setStatus(status: string): void {
    ...
    if (this._executionManager.status !== OperationStatus.Executing) {
  ...
  private _startWatching(): void {
    ...
    const sessionAbortSignal: AbortSignal = this._executionManager.abortController.signal;
    ...
    const operations: ReadonlySet<Operation> = this._executionManager.operations;
  ...
  private _onStdinData(chunk: string): void {
    const manager: IOperationExecutionManager = this._executionManager;
