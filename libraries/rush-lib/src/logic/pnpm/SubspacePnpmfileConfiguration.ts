...
export class SubspacePnpmfileConfiguration {
  ...
  public static getSubspacePnpmfileShimSettings(
    ...
    const projectNameToInjectedDependenciesMap: Map<
      string,
      Set<string>
    > = _getProjectNameToInjectedDependenciesMap(rushConfiguration, subspace);
...
function _getProjectNameToInjectedDependenciesMap(
  rushConfiguration: RushConfiguration,
  subspace: Subspace
  ...
  for (const subspaceProject of subspaceProjectsMap.values()) {
    ...
    const dependenciesMeta: IDependenciesMetaTable | undefined = subspaceProject.packageJson.dependenciesMeta;
    if (dependenciesMeta) {
...
function _processDependenciesForTransitiveInjectedInstall(
