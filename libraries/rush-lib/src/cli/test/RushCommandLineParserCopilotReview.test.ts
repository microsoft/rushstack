...
describe('old review baseline parser compatibility', () => {
  ...
  it.each([
    ...
    for (const reporting of [false, true]) {
      ...
      if (reporting) {
        const diagnostic: IRushDiagnostic = sink.events[1].payload as IRushDiagnostic;
