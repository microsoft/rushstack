...
export class Executable {
  ...
  public static spawnSync(
    ...
    const context: IExecutableContext = _getExecutableContext(options);

    const resolvedPath: string | undefined = _tryResolve(filename, options, context);
    if (!resolvedPath) {
  ...
  public static tryResolve(filename: string, options?: IExecutableResolveOptions): string | undefined {
    return _tryResolve(filename, options || {}, _getExecutableContext(options));
  }
}
...
function _tryResolve(
  ...
  if (hasPathSeparators) {
    ...
    return _tryResolveFileExtension(resolvedPath, context);
  } else {
    // Otherwise if it's a bare name, then try everything in the shell PATH
    const pathsToSearch: string[] = _getSearchFolders(context);

    for (const pathToSearch of pathsToSearch) {
      const resolvedPath: string = path.join(pathToSearch, filename);
      const result: string | undefined = _tryResolveFileExtension(resolvedPath, context);
      if (result) {
        return result;
      }
    }
...
function _tryResolveFileExtension(resolvedPath: string, context: IExecutableContext): string | undefined {
  if (_canExecute(resolvedPath, context)) {
    return resolvedPath;
  }
  ...
  for (const shellExtension of context.windowsExecutableExtensions) {
    ...
    if (_canExecute(resolvedNameWithExtension, context)) {
...
function _buildEnvironmentMap(options: IExecutableResolveOptions): EnvironmentMap {
 ...
 */
function _canExecute(filePath: string, context: IExecutableContext): boolean {
 ...
 */
function _getSearchFolders(context: IExecutableContext): string[] {
...
function _getExecutableContext(options: IExecutableResolveOptions | undefined): IExecutableContext {
  if (!options) {
    options = {};
  }

  const environment: EnvironmentMap = _buildEnvironmentMap(options);
 ...
 */
function _getEscapedForWindowsShell(text: string): string {
 ...
 */
function _validateArgsForWindowsShell(args: string[]): void {
...
function _buildCommandLineFixup(
  resolvedPath: string,
  args: string[],
  context: IExecutableContext
): ICommandLineOptions {
  ...
  if (OS_PLATFORM === 'win32') {
    ...
    switch (fileExtension.toUpperCase()) {
      ...
      case '.CMD': {
        ...
        shellArgs.push(_getEscapedForWindowsShell(resolvedPath));
        shellArgs.push(...args);
