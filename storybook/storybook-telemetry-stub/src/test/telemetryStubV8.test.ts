...
/**
 * Expected exports and their runtime types for \@storybook/telemetry v8.
 */
const EXPECTED_V8_EXPORTS: Record<string, string> = {
  addToGlobalContext: 'function',
  cleanPaths: 'function',
  computeStorybookMetadata: 'function',
  getPrecedingUpgrade: 'function',
  getStorybookMetadata: 'function',
  isExampleStoryId: 'function',
  metaFrameworks: 'object',
  oneWayHash: 'function',
  removeAnsiEscapeCodes: 'function',
...
describe('storybook-telemetry-stub (v8 compatibility)', () => {
  ...
  beforeAll(() => {
    // The aliased devDependency "@storybook/telemetry-8" resolves to @storybook/telemetry@8.x
    officialExports = require('@storybook/telemetry-8');
  });
  ...
  it('should have matching runtime types for all v8 exports', () => {
    for (const [name, expectedType] of Object.entries(EXPECTED_V8_EXPORTS)) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
