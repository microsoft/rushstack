  ...
  if (options.regexName) {
    ...
    lines.push(
      ...[
        ...
        `  var match = url && url.match(regex);`,
        `  if (match) {`,
        `    ${varName} = url.slice(0, match.index);`,
        `  }`,
        `}`,
        '',
        ...
        `    var match = path && path.match(regex);`,
        '    if (match) {',
        `      ${varName} = path.slice(0, match.index);`,
        ...
        `    var match = global && global.match(regex);`,
        '    if (match) {',
        `      ${varName} = global.slice(0, match.index);`,
        ...(options.preferLastFoundScript ? [] : ['      break;']),
        '    }',
        '  }',
        '}'
