  ...
  implements IHeftTaskPlugin<IVSCodeExtensionVerifySignaturePluginOptions>
{
  public apply(
    ...
    heftTaskSession.hooks.run.tapPromise(PLUGIN_NAME, async (runOptions: IHeftTaskRunHookOptions) => {
      ...
      terminal.writeLine(`VerifySignatureing VSIX ${vsixPath}`);
      ...
      if (verifySignatureResult.exitCode !== 0) {
        throw new Error(`VSIX verify-signatureing failed with exit code ${verifySignatureResult.exitCode}`);
      }
      terminal.writeLine('VSIX successfully verify-signatureed.');
    });
  }
}
