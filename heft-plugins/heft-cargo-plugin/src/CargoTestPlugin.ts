...
interface ICargoTestConfiguration {
  release?: boolean;
  workspace?: boolean;
}

const PLUGIN_NAME: 'cargo-plugin' = 'cargo-plugin';
const RELEASE_PARAMETER_LONG_NAME: '--release' = '--release';
const WORKSPACE_PARAMETER_LONG_NAME: '--workspace' = '--workspace';
const CONFIG_FILE_NAME: string = 'cargo-test.json';
...
export default class CargoTestPlugin implements IHeftTaskPlugin<ICargoTestPluginOptions> {
  ...
  public apply(
    ...
    taskSession.hooks.run.tapPromise(PLUGIN_NAME, async (runOptions: IHeftTaskRunHookOptions) => {
      await this._runCargoTestAsync(taskSession, heftConfiguration, options, runOptions.abortSignal);
    });
  }
  ...
  private async _loadConfigurationAsync(
    ...
    options: ICargoTestPluginOptions,
    taskSession: IHeftTaskSession
  ): Promise<ICargoTestConfiguration> {
    // Try to load configuration from config/cargo-test.json
    const configPath: string = path.join(heftConfiguration.buildFolderPath, 'config', CONFIG_FILE_NAME);

    let config: ICargoTestConfiguration = {};
  ...
  private async _runCargoTestAsync(
    ...
    const config: ICargoTestConfiguration = await this._loadConfigurationAsync(
      heftConfiguration,
      options,
      taskSession
    );
    ...
    if (this._workspaceMode) {
      taskSession.logger.terminal.writeLine('Running tests for all workspace packages');
    }
