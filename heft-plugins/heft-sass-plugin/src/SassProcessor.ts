...
export interface ISassProcessorOptions {
  ...
  /**
   * If specified, theming expressions like `[theme: someVariable, default: #fff]` will be converted to CSS variable expressions like `var(--someVariable, #fff)`.
   */
  convertLoadStylesToCssVariables?: boolean;
}
...
export class SassProcessor {
  ...
  private async _compileFileAsync(
    ...
    const {
      ...
      srcFolder,
      exportAsDefault,
      convertLoadStylesToCssVariables
    } = this._options;
    ...
    if (convertLoadStylesToCssVariables) {
      css = replaceTokensWithVariables(css);
    }
