...
export class HeftTask implements IHeftTask {
  ...
  #consumingTasks: Set<HeftTask> | undefined;
  #dependencyTasks: Set<HeftTask> | undefined;

  #taskPluginDefinition: HeftTaskPluginDefinition | undefined;
  #taskPlugin: IHeftTaskPlugin | undefined;
  ...
  public get consumingTasks(): ReadonlySet<HeftTask> {
    if (!this.#consumingTasks) {
      ...
      const { tasks } = this.#parentPhase;

      for (const task of tasks) {
        task.#consumingTasks = new Set();
      }
      for (const task of tasks) {
        for (const dependency of task.dependencyTasks) {
          dependency.#consumingTasks!.add(task);
        }
      }
    }
  ...
  public get pluginDefinition(): HeftTaskPluginDefinition {
    ...
    return this.#taskPluginDefinition;
  }

  public get pluginOptions(): object | undefined {
    return this.#taskSpecifier.taskPlugin.options;
  }

  public get dependencyTasks(): Set<HeftTask> {
    if (!this.#dependencyTasks) {
      this.#dependencyTasks = new Set();
      const dependencyNamesSet: Set<string> = new Set(this.#taskSpecifier.taskDependencies || []);

      for (const dependencyName of dependencyNamesSet) {
        // Skip if we can't find the dependency
        const dependencyTask: HeftTask | undefined = this.#parentPhase.tasksByName.get(dependencyName);
        if (!dependencyTask) {
          throw new Error(
            `Could not find dependency task ${JSON.stringify(dependencyName)} within phase ` +
              `${JSON.stringify(this.#parentPhase.phaseName)}.`
          );
        }
  ...
  public async getPluginAsync(logger: IScopedLogger): Promise<IHeftTaskPlugin<object | void>> {
    ...
    if (!this.#taskPlugin) {
      this.#taskPlugin = await this.#taskPluginDefinition!.loadPluginAsync(logger);
    }
    ...
    if (!isTaskPluginDefinition) {
      throw new Error(
        `Plugin ${JSON.stringify(pluginSpecifier.pluginName)} specified by task ` +
          `${JSON.stringify(this.#taskName)} is not a task plugin.`
      );
    }
  ...
  private _validate(): void {
    ...
    if (!this.#taskSpecifier.taskPlugin) {
