...
inventing a cached module path. Warm-set and retry support do not establish availability
of another selected Rush version: a matching daemon launcher must genuinely be resolved,
otherwise restart fails closed. Older Rush versions may also reject the new `daemon` config block; use
