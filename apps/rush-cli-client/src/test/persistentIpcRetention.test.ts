...
function measuredRank(warm: IDaemonWarmSetStatus, name: ProjectName): IMeasuredRank {
  ...
  return {
    ...rank,
    timeSavedMs: rank.timeSavedMs,
    measuredRunnerMemoryBytes: rank.measuredRunnerMemoryBytes
  };
}
...
describe('measured production IPC retention through the public client', () => {
  ...
  it.each([
    ...
    'uses actual score versus opposite recency under real memory pressure (telemetry: %s, cold work: %s)',
    (telemetry, coldWork) => {
      ...
      return fixture.runAsync(async () => {
        ...
        if (fixture.events().filter((event) => event.kind === 'complete').length !== 4) {
          throw new Error(
            `Expected four real setup executions: ${JSON.stringify({ initial, events: fixture.events() })}`
          );
        }
        ...
        if (initialHigh.lastUsed >= initialLow.lastUsed || score(initialHigh) === score(initialLow)) {
          const lowScoreAfterHit =
            (initialLow.timeSavedMs * (initialLow.frequency + 1)) / initialLow.measuredRunnerMemoryBytes;
        ...
        expect(high.frequency).toBe(
          initialHigh.frequency + cacheHitRequests.filter((name) => name === higherValue).length
        );
        expect(low.frequency).toBe(
          initialLow.frequency + cacheHitRequests.filter((name) => name === newer).length
        );
        ...
        for (const rank of [a, b]) {
          const sample = fixture
            .events()
            .filter((event) => event.project === rank.projectName && event.kind === 'complete')
            .at(-1);
        ...
        try {
          ...
          const waitForPressureSampleAsync = async (
            ...
            while (
              ...
              if (Date.now() >= readyDeadline)
                throw new Error(`The real pressure operation did not reach ${kind}.`);
              await delayAsync(10);
            }
          ...
          allocation = getPressureAllocationBytes(
            sampleMemory(pressureAccounting, ready.residentMemoryBytes)
          );
          ...
          pressureAdjustment = getPressureAllocationBytes(
            sampleMemory(await readPressureAccountingAsync(), allocated.residentMemoryBytes)
          );
          writeIpcFixtureFile(
            `${pressureGate}.adjust`,
            JSON.stringify({ additionalMemoryBytes: pressureAdjustment })
          );
          ...
          expect(retainedBytes + verifiedPressure.otherRunnerBytes).toBeGreaterThan(
            verifiedPressure.budgetBytes
          );
        ...
        const pressure = fixture
          .events()
          .filter((event) => event.project === newer && event.kind === 'complete')
          .at(-1);
        ...
        const evictedPid = fixture
          .events()
          .find((event) => event.project === evict && event.kind === 'ready')!.pid;
        expect(fixture.events().some((event) => event.kind === 'closed' && event.pid === evictedPid)).toBe(
          true
        );
        expect(fixture.events().filter((event) => event.kind === 'ready')).toHaveLength(2);
