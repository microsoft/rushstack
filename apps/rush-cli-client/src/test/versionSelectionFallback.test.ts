...
it('uses the native version selector when no compatible foreign daemon launcher exists', async () => {
  ...
  const selection = jest
    .spyOn(connectionOptions, 'getDaemonConnectionOptionsAsync')
    .mockRejectedValue(new DaemonLauncherUnavailableError('5.178.1', 'No compatible request-launch APIs.'));
  ...
  try {
    ...
    expect(selection).toHaveBeenCalledWith(
      folder,
      '5.178.1',
      expect.objectContaining({ RUSH_DAEMON: '1' }),
      true
    );
