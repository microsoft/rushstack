...
describe(ClientOperationRenderer.name, () => {
  it('renders operation chrome and global stderr activity without adding a second newline', async () => {
    ...
    const renderer = new ClientOperationRenderer({
      ...
      writeAsync: async (bytes, stream) => {
        output[stream] += Buffer.from(bytes).toString();
      }
    });
    ...
    await renderer.writeEventAsync(
      event('extension', {
        name: RUSHD_OPERATION_STREAM_CLOSED,
        data: { operationId: 'project' }
      })
    );
