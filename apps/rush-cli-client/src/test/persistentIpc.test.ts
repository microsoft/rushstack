...
describe('explicit persistent Node IPC through the public client', () => {
  ...
  it('runs the real Node launcher, preserves raw args, reuses PID and skips unchanged work', () =>
    withFixtureAsync(async (fixture) => {
      const result = await fixture.invokeAsync([
        'build',
        '--only',
        'a',
        '--label',
      ...
      expect(events.filter((event) => event.kind === 'complete').map((event) => event.pid)).toEqual([
        ready[0].pid,
        ready[0].pid
      ]);
      expect((await fixture.statusAsync()).workspace?.generationToken).toBe(
        status.workspace?.generationToken
      );
      expect(fs.readFileSync(path.join(fixture.folder, 'a/lib/result.txt'), 'utf8')).toBe('two:original');
    }));

  it('retires entrypoint and local tool implementation changes without restarting on ordinary input', () =>
    withFixtureAsync(async (fixture) => {
  ...
  it('leaves non-opted-in builds and rebuilds on the native shell path', () =>
    withFixtureAsync(
      async (fixture) => {
        await fixture.buildAsync('--only', 'a');
