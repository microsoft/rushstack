...
describe('production Node descriptor boundaries through the public client', () => {
  ...
  it.each(['direct', 'inherited', 'rig'] as const)(
    'replaces a changed %s descriptor before reusing a child',
    (kind) =>
      withFixtureAsync(
        async (fixture) => {
          ...
          expect(
            fixture.events().some((event) => event.kind === 'closed' && event.pid === previous.pid)
          ).toBe(true);
        },
  ...
  it('preserves preassigned native shard runners instead of applying the Node descriptor to them', () =>
    withFixtureAsync(async (fixture) => {
      ...
      const config = JSON.parse(
        fs.readFileSync(path.join(fixture.folder, 'a/config/rush-project.json'), 'utf8')
      );
  ...
  it('honors native --no-ipc where registered without enabling graph watch mode', () =>
    withFixtureAsync(async (fixture) => {
      const commands = JSON.parse(
        fs.readFileSync(path.join(fixture.folder, 'common/config/rush/command-line.json'), 'utf8')
      );
      commands.commands[0].watchOptions = {
        alwaysWatch: false,
        watchPhases: ['_phase:compile', '_phase:empty']
      };
  ...
  it('preserves complete Unicode output and warning/failure results from the real retained tool', () =>
    withFixtureAsync(async (fixture) => {
      ...
      expect(warning.stdout.slice(beginning + 'PAYLOAD_BEGIN:'.length, ending)).toBe(
        String.fromCodePoint(0x1f642).repeat(32_769)
      );
  ...
  it('does not replay a launched descriptor that exits without IPC readiness', () =>
    withFixtureAsync(async (fixture) => {
      fixture.write(
        'a/tools/ipc/entry.cjs',
        "require('node:fs').appendFileSync('../common/temp/no-ipc-runs', 'once\\n');\n"
      );
