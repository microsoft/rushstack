...
describe('native mutations through the standalone CLI and default daemon', () => {
  it.each(['install', 'update'] as const)(
    'runs %s without fallback or administrative replay',
    async (command) => {
      ...
      fixture.paths = resolveDaemonPaths(
        ...
        computeDaemonWorkspaceKey({
          canonicalRepoRoot: fs.realpathSync.native(fixture.repoRoot),
      ...
      try {
        ...
        expect(
          await invokeAsync(fixture, ['build', '--to', '@mutation/app', '--parallelism', '3'])
        ...
        const rebuilt: IClientResult = await invokeAsync(fixture, [
          'rebuild',
          '--to',
          '@mutation/app',
          '--parallelism',
        ...
        expect(fixture.readAppOutput()).toBe(
          `${command === 'install' ? 'provider-one' : 'provider-two'}-installed:before`
        );
