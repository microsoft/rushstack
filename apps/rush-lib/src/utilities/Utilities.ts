 ...
 */
export interface IInstallPackageInDirectoryOptions extends IInstallPackageBaseOptions {
  packageName: string;
  version: string;
}

export interface IPackageSelector {
  name: string;
  version: string;
}

/**
 * Options for Utilities.installPackagesInDirectory().
 */
export interface IInstallPackagesInDirectoryOptions extends IInstallPackageBaseOptions {
  packages: IPackageSelector[];
}

export interface IInstallPackageBaseOptions {
  directory: string;
...
export class Utilities {
  ...
  public static installPackageInDirectory(options: IInstallPackageInDirectoryOptions): void {
    Utilities.installPackagesInDirectory({
      ...
      tempPackageTitle: options.tempPackageTitle,
      maxInstallAttempts: options.maxInstallAttempts,
      commonRushConfigFolder: options.commonRushConfigFolder,
  ...
  /**
   * Installs packages (specified with names and versions) in the specified directory.
   */
  public static installPackagesInDirectory(options: IInstallPackagesInDirectoryOptions): void {
    ...
    for (const pkg of options.packages) {
      npmPackageJson.dependencies![pkg.name] = pkg.version;
    }
