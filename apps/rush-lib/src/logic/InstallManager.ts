...
export class InstallManager {
  ...
  public constructor(
    ...
    this._commonNodeModulesMarker = new LastInstallFlag(
  ...
  public async doInstall(): Promise<void> {
    ...
    if (!shrinkwrapIsUpToDate) {
      if (!options.allowShrinkwrapUpdates) {
        console.log();
        console.log(
          colors.red(`The ${this._shrinkwrapFilePhrase} is out of date. You need to run "rush update".`)
        );
        throw new AlreadyReportedError();
      }
    }
    ...
    for (const rushProject of sortedRushProjects) {
      ...
      for (const [packageName, packageVersion] of tempDependencies.entries()) {
        ...
        if (this._rushConfiguration.packageManager === 'pnpm') {
          ...
          tryReusingPackageVersionsFromShrinkwrap = !this._rushConfiguration.experimentsConfiguration
    ...
    return Promise.resolve().then(() => {
      ...
      return this._checkIfReleaseIsPublished()
        ...
        .then((publishedRelease: boolean | undefined) => {
          ...
          packageManagerEnv = this._mergeEnvironmentVariables(process.env, configurationEnvironment);
          ...
          if (options.allowShrinkwrapUpdates && (usePnpmFrozenLockfile || !shrinkwrapIsUpToDate)) {
            ...
            const tempShrinkwrapFile:
              | BaseShrinkwrapFile
              | undefined = ShrinkwrapFileFactory.getShrinkwrapFile(
              this._rushConfiguration.packageManager,
              this._rushConfiguration.packageManagerOptions,
