...
describe('npm ShrinkwrapFile', () => {
  ...
  it('verifies temp project dependencies', () => {
    // Found locally
    expect(
      shrinkwrapFile.tryEnsureCompatibleDependency('jquery', '>=2.2.4 <3.0.0', '@rush-temp/project2')
    ).toEqual(true);
    // Found at root
...
describe('pnpm ShrinkwrapFile', () => {
  ...
  it('verifies temp project dependencies', () => {
    expect(
      shrinkwrapFile.tryEnsureCompatibleDependency('jquery', '>=2.0.0 <3.0.0', '@rush-temp/project1')
    ).toEqual(true);
    expect(shrinkwrapFile.tryEnsureCompatibleDependency('q', '~1.5.0', '@rush-temp/project2')).toEqual(true);
    expect(shrinkwrapFile.tryEnsureCompatibleDependency('left-pad', '~9.9.9', '@rush-temp/project1')).toEqual(
      false
    );
    expect(
  ...
  it('can reuse the latest version that another temp package is providing', () => {
    expect(
      shrinkwrapFile.tryEnsureCompatibleDependency('jquery', '>=2.0.0 <3.0.0', '@rush-temp/project3')
    ).toEqual(true);
  });
});
...
describe('extractVersionFromPnpmVersionSpecifier', () => {
  ...
  it('extracts a scoped peer dep', () => {
    expect(extractVersionFromPnpmVersionSpecifier('/@ms/sp-client-utilities/3.1.1/foo@13.1.0')).toEqual(
      '3.1.1'
    );
  });
  it('extracts relative versions', () => {
    expect(
      extractVersionFromPnpmVersionSpecifier('example.pkgs.visualstudio.com/@scope/testDep/1.0.0')
    ).toEqual('1.0.0');
    expect(
      extractVersionFromPnpmVersionSpecifier('example.pkgs.visualstudio.com/@scope/testDep/1.2.3-beta.3')
  ...
  it('handles bad cases', () => {
    ...
    expect(extractVersionFromPnpmVersionSpecifier('//')).toEqual(undefined);
    expect(extractVersionFromPnpmVersionSpecifier('/@/')).toEqual(undefined);
    expect(extractVersionFromPnpmVersionSpecifier('example.pkgs.visualstudio.com/@scope/testDep/')).toEqual(
      undefined
    );
  });
});
