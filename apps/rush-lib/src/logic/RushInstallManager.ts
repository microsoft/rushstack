...
export class RushInstallManager extends BaseInstallManager {
  ...
  protected async prepareAndCheckShrinkwrap(
    ...
    const tempProjectsFolder: string = path.join(this.rushConfiguration.commonTempFolder,
      ...
      for (const [packageName, packageVersion] of tempDependencies.entries()) {
        ...
        if (shrinkwrapFile) {
          if (
            ...
            shrinkwrapWarnings.push(`Missing dependency "${packageName}" (${packageVersion}) required by`
  ...
  protected canSkipInstall(lastModifiedDate: Date): boolean {
    console.log(os.EOL + colors.bold('Checking node_modules in ' + this.rushConfiguration.commonTempFolder)
      + os.EOL);
    ...
    potentiallyChangedFiles.push(path.join(this.rushConfiguration.commonTempFolder, 'node_modules'));
    ...
    // Example: "C:\MyRepo\common\temp\projects\my-project-2.tgz"
    potentiallyChangedFiles.push(...this.rushConfiguration.projects.map(x => this._getTarballFilePath(x)));
  ...
  protected async install(cleanInstall: boolean) : Promise<void> {
    ...
    if (this.rushConfiguration.packageManager === 'yarn') {
      ...
      const yarnRushTempCacheFolder: string = path.join(
        this.rushConfiguration.yarnCacheFolder, 'v2', 'npm-@rush-temp'
      );
    ...
    if (this.options.debug || this.options.collectLogFile || this.options.networkConcurrency) {
      console.log(os.EOL + colors.green('Invoking package manager: ')
        + FileSystem.getRealPath(this.rushConfiguration.packageManagerToolFilename) + ' '
        + installArgs.join(' ') + os.EOL);
    }
  ...
  private _fixupNpm5Regression(): void {
    const pathToDeleteWithoutStar: string = path.join(this.rushConfiguration.commonTempFolder,
    ...
    for (const tempProjectName of shrinkwrapFile.getTempProjectNames()) {
      if (!this.rushConfiguration.findProjectByTempName(tempProjectName)) {
        console.log(os.EOL + colors.yellow(Utilities.wrapWords(
          `Your ${this.rushConfiguration.shrinkwrapFilePhrase} references a project "${tempProjectName}" `
