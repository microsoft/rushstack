...
import {
  convertLockfileV6DepPathToV5DepPath,
  getShrinkwrapFileMajorVersion
} from '../../../utils/shrinkwrap';
import type { ExplorerCommandLineParser } from '../ExplorerCommandLineParser';
...
export class StartAction extends CommandLineAction {
  ...
  protected async onExecute(): Promise<void> {
    ...
    updateNotifier({
      ...
      updateCheckInterval: this._isDebug ? 0 : undefined
    }).notify({
    ...
    app.listen(PORT, async () => {
      ...
      if (!appState.debugMode) {
        try {
          ...
          this._terminal.writeError('Error launching browser: ' + e.toString());
        }
      }
    });
  }
