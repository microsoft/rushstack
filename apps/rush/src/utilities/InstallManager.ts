...
export default class InstallManager {
  ...
  public ensureLocalPnpmTool(forceReinstall: boolean): void {
    ...
    const pnpmToolFolder: string = path.join(rushHomeFolder, 'pnpm-' + this._rushConfiguration.npmToolVersion);
    ...
    if (forceReinstall || !fsx.existsSync(pnpmToolFlagFile)) {
      console.log(colors.bold('Installing PNPM version ' + this._rushConfiguration.npmToolVersion) + os.EOL);
      ...
      const pnpmPackageJson: IPackageJson = {
        dependencies: { 'pnpm': this._rushConfiguration.npmToolVersion },
      ...
      console.log('Successfully installed PNPM ' + this._rushConfiguration.npmToolVersion);
    } else {
      console.log('Found PNPM version ' + this._rushConfiguration.npmToolVersion + ' in ' + pnpmToolFolder);
    }
