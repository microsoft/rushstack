...
export default class InstallManager {
   ...
   * The file itself may not actually exist.  If the file exists, it means a "pnpm install"
  ...
  /**
   * If the "pnpm-local" symlink hasn't been set up yet, this creates it, installing the
   * specified PNPM version in the user's home directory if needed.
   */
  public ensureLocalPnpmTool(forceReinstall: boolean): void {
    ...
    const pnpmToolFolder: string = path.join(rushHomeFolder, 'pnpm-' + this._rushConfiguration.pnpmToolVersion);
    // Example: "C:\Users\YourName\.rush\pnpm-1.2.3\last-install.flag"
    const pnpmToolFlagFile: string = path.join(pnpmToolFolder, 'last-install.flag');

    // NOTE: We don't care about the timestamp for last-install.flag, because nobody will change
    // the package.json for this case
    if (forceReinstall || !fsx.existsSync(pnpmToolFlagFile)) {
      console.log(colors.bold('Installing PNPM version ' + this._rushConfiguration.pnpmToolVersion) + os.EOL);

      if (fsx.existsSync(pnpmToolFolder)) {
        console.log('Deleting old files from ' + pnpmToolFolder);
        Utilities.dangerouslyDeletePath(pnpmToolFolder);
      }
      Utilities.createFolderWithRetry(pnpmToolFolder);

      const pnpmPackageJson: IPackageJson = {
        dependencies: { 'pnpm': this._rushConfiguration.pnpmToolVersion },
      ...
      console.log('Successfully installed PNPM ' + this._rushConfiguration.pnpmToolVersion);
    } else {
      console.log('Found PNPM version ' + this._rushConfiguration.pnpmToolVersion + ' in ' + pnpmToolFolder);
    }
    ...
    const localPnpmToolFolder: string = path.join(this._rushConfiguration.commonTempFolder, 'pnpm-local');
    if (fsx.existsSync(localPnpmToolFolder)) {
      fsx.unlinkSync(localPnpmToolFolder);
    }
    console.log(os.EOL + 'Symlinking "' + localPnpmToolFolder + '"');
    console.log('  --> "' + pnpmToolFolder + '"');
    fsx.symlinkSync(pnpmToolFolder, localPnpmToolFolder, 'junction');
  }
  ...
  /**
   * Runs "pnpm install" in the common folder, in one of three ways:
   * 1. No action because it is already up to date
   * 2. Incremental action ("pnpm prune", "pnpm install", etc).
   * 3. Full clean and "pnpm install"
   */
  public installCommonModules(installType: InstallType): void {
    ...
    if (!fsx.existsSync(pnpmToolFilename)) {
      ...
      throw new Error('Expected to find local PNPM here: "' + pnpmToolFilename + '"');
    }
    ...
    if (fsx.existsSync(commonNodeModulesFolder)) {
      ...
      if (deletingNodeModules) {
        ...
        console.log(`Running "pnpm prune" in ${this._rushConfiguration.commonTempFolder}`);
        const pnpmPruneArgs: string[] = ['prune'];
        this.pushConfigurationNpmArgs(pnpmPruneArgs);
        Utilities.executeCommandWithRetry(pnpmToolFilename, pnpmPruneArgs, MAX_INSTALL_ATTEMPTS,
          this._rushConfiguration.commonTempFolder);
    ...
    console.log(os.EOL + colors.bold(`Running "pnpm install" in ${this._rushConfiguration.commonTempFolder}`)
      + os.EOL);
    Utilities.executeCommandWithRetry(pnpmToolFilename,
      pnpmInstallArgs,
