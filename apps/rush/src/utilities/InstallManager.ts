...
export default class InstallManager {
  ...
  public ensureLocalPackageManager(forceReinstall: boolean): void {
    ...
    if (forceReinstall || !fsx.existsSync(packageManagerToolFlagFile)) {
      ...
      if (fsx.existsSync(packageManagerToolFolder)) {
        console.log('Deleting old files from ' + packageManagerToolFolder);
        Utilities.dangerouslyDeletePath(packageManagerToolFolder);
      }
      Utilities.createFolderWithRetry(packageManagerToolFolder);

      const npmPackageJson: IPackageJson = {
        dependencies: { [packageManager]: packageManagerVersion },
        description: 'Temporary file generated by the Rush tool',
        name: 'npm-local-install',
        private: true,
        version: '0.0.0'
      };
      JsonFile.save(npmPackageJson, path.join(packageManagerToolFolder, 'package.json'));

      console.log(os.EOL + 'Running "npm install" in ' + packageManagerToolFolder);

      // NOTE: Here we use whatever version of NPM we happen to find in the PATH
      Utilities.executeCommandWithRetry('npm', ['install'], MAX_INSTALL_ATTEMPTS, packageManagerToolFolder);
