...
This log was last generated on Sat, 30 Jun 2018 00:57:22 GMT and should not be manually modified.
...
- Update peerDependencies when bumping package versions
- Implement allowedAlternativeVersions
- Fix to handle pnpm relative\absolute dependency path version
- Print each project's build time during the summary.
- Fix a performance regression for "rush build"
- Fix a regression where rush could exit with zero exit code with build failures.
- Fix a minor grammar error in a rush logging message.
- Fix an issue where the /common/temp/.npmrc file could contain missing environment variable tokens.
- Create install-run.js and intsall-run-rush.js to invoke tools in CI.
- Make git optional for rush (re)build.
- Minor fix to the ci-rush script.
- Ensure install-run runs "npm view" in a folder with an .npmrc.
- Remove the experimental --auth-token parameter in favor of tokens in environment variables.
- Add --auth-token and RUSH_TEMP_FOLDER options to Rush.
- Introduce a script to install rush for CI.
...
- Fix an issue where the common/local-rush symlink sometimes failed to be recreated
- Fix a regression where the package manager was not installed correctly
- Update rush.json schema to support prerelease versions of Rush
- Fix annoyance where "rush generate" would always change the integrity hash for tarball entries in shrinkwrap.yaml 
...
- Fix a regression where builds would sometimes fail with a zero exit code
...
- Fix minor regression for "rush rebuild"
- Replace pinned-versions.json with a more general common-versions.json that can track other cross-project dependency versions
- Rename "pinned versions" concept to "preferred versions", and separate the XStitch versions into their own field
- Add the new "allowedAlternativeVersions" setting (not implemented yet)
...
- Add missing slash for "shrinkwrap-preinstall.yaml" file path
- Remove unused RushConfiguration.homeFolder API
...
- Rush now manages some standard scripts in the "common/scripts" folder
- When installing tools, always copy the repo's .npmrc file to the target folder
- Replace the "rush generate" command with a new command "rush update"
- Replace "rush install --clean" and "--full-clean" with "rush install --purge"
...
- Eliminate extra letters from shortened command line options (renamed "-cpo" to "-o", and removed "-vp" and "-pn")
- Add check for unpublished releases
- Support "rush rebuild" to build all projects in the specified version policy
- Should respect environment when checking if npm packages exist
