 ...
 * Modes:
 * - pack:
 *   - Recursively finds files (directories not stored as entries).
 *   - Computes CRC32 (ZIP) and SHA-1 (change detection) for each file.
 *   - Optionally deflates files (method 8) only if compression makes them smaller; else stored (method 0).
 *   - Adds a metadata JSON entry (__zipsync_metadata__.json) listing each file's size and SHA‑1.
 *   - Writes local headers + data, then central directory, then EOCD. (No ZIP64, comments, or append.)
 *   - Empty directories are not recorded in the archive.
 *
 * - unpack:
 *   - Reads EOCD and central directory to enumerate entries.
 *   - Loads metadata entry for authoritative manifest.
 *   - For each non-metadata entry:
 *       * If a local file exists with matching size + SHA‑1, skip writing.
 *       * Otherwise extract (inflate if needed).
 *   - Creates directories on demand (none stored in archive).
 *   - Deletes local files not present in the archive manifest.
 *   - Empty directories are removed from the target folder.
 *
 * @public
 */
export function zipSync<T extends IZipSyncOptions>(
  ...
  function packZip(): IPackResult {
    ...
    const metadataEntry: IFileEntry = {
      ...
      crc32: zlib.crc32(metadataBuffer),
