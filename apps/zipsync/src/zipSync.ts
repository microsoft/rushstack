...
function unpackZip({ archivePath, baseDir, targetDirectories, terminal }: IZipSyncOptions): IUnpackResult {
...
export function zipSync<T extends IZipSyncOptions>(
  ...
  const baseDir: string = (options.baseDir = path.resolve(rawBaseDir));
  options.targetDirectories = rawTargetDirectories.map((dir) => path.join(baseDir, dir));

  if (mode === 'pack') {
    terminal.writeLine(`Packing to ${archivePath} from ${rawTargetDirectories.join(', ')}`);
    return packZip(options) as T['mode'] extends 'pack' ? IPackResult : IUnpackResult;
  } else {
    terminal.writeLine(`Unpacking to ${rawTargetDirectories.join(', ')} from ${archivePath}`);
    return unpackZip(options) as T['mode'] extends 'pack' ? IPackResult : IUnpackResult;
  }
}
